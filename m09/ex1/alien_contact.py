...
class AlienContactReport(BaseModel):
    ...
    def validation_rules(self) -> "AlienContactReport":
        ...
        if self.contact_type == Contact_type.PHYSICAL and self.is_verified == True:
            raise ValueError("Physical contacts must be verified")
        if self.contact_type == Contact_type.TELEPHATIC and self.witness_count < 3:
            raise ValueError("Telephatic contact requires at least 3 witnesses")

        if self.signal_strength >= 7.0 and not self.message_received:
            raise ValueError("Strong signals (signal_strength > 7.0) must include message_received.")
        return self
